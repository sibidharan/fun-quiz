...
1. **Use Disabled Superglobals**: For new projects, disable superglobals to leverage full coroutine support
2. **Use G::instance()**: Always use the G singleton for accessing request data in coroutine mode
3. **Implicit Routes for Simple Pages**: Use `public/` directory for straightforward pages
4. **Explicit Routes for Complex Logic**: Use `$app->route()` for routes requiring parameters or middleware
5. **API Structure**: Keep APIs in the `api/` directory for automatic routing
6. **Template Organization**: Group related templates in subdirectories matching their PHP files
7. **Middleware for Cross-Cutting Concerns**: Use PSR-15 middleware for auth, logging, CORS, etc.
